...
struct ListenStateMachine {
    ...
    mutating func startListeningFailed(channel: String, error: some Error) -> Dictionary<Int, NotificationListener>.Values {
    ...
    mutating func fail(_ error: some Error) -> [NotificationListener] {
...
extension ListenStateMachine {
    struct ChannelState {
        ...
        mutating func startListeningFailed(_ error: some Error) -> Dictionary<Int, NotificationListener>.Values {
            switch self.state {
            case .initialized, .listening, .stopping:
                fatalError("Invalid state: \(self.state)")
        ...
        mutating func fail(_ error: some Error) -> FailAction {
            switch self.state {
            case .initialized:
                fatalError("Invalid state: \(self.state)")
