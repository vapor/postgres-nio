...
struct PostgresMessageDecodingError: Error {
    ...
    static func unknownMessageIDReceived(
        ...
        return PostgresMessageDecodingError(
            ...
            payload: data.base64EncodedString(),
            description:
                "Received a message with messageID '\(Character(UnicodeScalar(messageID)))'. There is no message type associated with this message identifier.",
            file: file,
            line: line)
    }
...
extension ByteBuffer {
    mutating func throwingReadInteger<I: FixedWidthInteger>(as: I.Type, file: String = #fileID, line: Int = #line) throws -> I {
        guard let result = self.readInteger(endianness: .big, as: I.self) else {
            throw PSQLPartialDecodingError.expectedAtLeastNRemainingBytes(
                MemoryLayout<I>.size, actual: self.readableBytes, file: file, line: line)
        }
        return result
    }
