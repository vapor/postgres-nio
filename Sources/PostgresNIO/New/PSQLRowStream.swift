...
final class PSQLRowStream: @unchecked Sendable {
    ...
    private func receiveError(_ error: some Error) {
        switch self.downstreamState {
    ...
    private func executeActionBasedOnYieldResult(_ yieldResult: AsyncSequenceSource.YieldResult, source: some PSQLRowsDataSource) {
        self.eventLoop.preconditionInEventLoop()
        switch yieldResult {
        case .dropped:
            // ignore
