...
public struct PSQLError: Error, @unchecked Sendable {
    ...
    static func sasl(underlying: some Error) -> PSQLError {
        var error = PSQLError(code: .saslError)
        error.underlying = underlying
        return error
    }

    static func failedToAddSSLHandler(underlying: some Error) -> PSQLError {
        var error = PSQLError(code: .failedToAddSSLHandler)
        error.underlying = underlying
        return error
    }

    static func connectionError(underlying: some Error) -> PSQLError {
    ...
    static func unlistenError(underlying: some Error) -> PSQLError {
