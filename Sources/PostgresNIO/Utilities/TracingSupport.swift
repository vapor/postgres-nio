...
struct TracingSupport: Sendable {
    ...
    func withSpan<T>(
        ...
        // `TaskLocal.withValue` does not forward a `sending` result, so the value is carried out of the
        // closure in an unchecked box. This is safe: the closure completes on the current task before
        // the value is unboxed, so it never crosses an isolation boundary.
