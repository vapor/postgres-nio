...
extension UInt8 {
    fileprivate static var comma: UInt8 {
        0x2c  // .init(ascii: ",")
    }
    fileprivate static var equals: UInt8 {
        0x3d  // .init(ascii: "=")
    }
}
...
extension String {
    ...
    /// ````
    fileprivate var decodedAsSaslName: String? {
    ...
    fileprivate var encodedAsSaslName: String {
        return self.replacingOccurrences(of: ",", with: "=2C").replacingOccurrences(of: "=", with: "=3D")
    }

    fileprivate init?(printableAscii: [UInt8]) {
    ...
    fileprivate init?(asciiAlphanumericMorse: [UInt8]) {
...
extension Array where Element == UInt8 {
    ...
    fileprivate func decodingBase64() -> [UInt8]? {
        var actual = self
        if actual.count % 4 != 0 {
    ...
    fileprivate func encodingBase64() -> [UInt8] {
        return Array(
    ...
    /// `1*(ALPHA / DIGIT / "." / "-")`
    fileprivate var isAsciiAlphanumericMorse: Bool {
    ...
    /// ````
    fileprivate var isValidScramValue: Bool {
...
private enum SCRAMServerError: Error, RawRepresentable {
    ...
    case invalidEncoding, extensionsNotSupported, invalidProof, channelBindingsDontMatch,
        ...
        unknownUser, invalidUsernameEncoding, noResources, otherError
    case serverErrorValueExt(String)
...
private struct SCRAMMessageParser {
    static func parseAttributePair(name: [UInt8], value: [UInt8], isGS2Header: Bool = false) -> SCRAMAttribute? {
        ...
        case UInt8(ascii: "c") where !isGS2Header:
            ...
            case (.gp(.bind(let name, .none)), .a(let ident), .gm(let data)): return .c(binding: .bind(name, data), authIdentity: ident)
            case (.gp(.bind(let name, .none)), .gm(let data), .none): return .c(binding: .bind(name, data))
            case (.gp(let bind), .a(let ident), .none): return .c(binding: bind, authIdentity: ident)
            case (.gp(let bind), .none, .none): return .c(binding: bind)
            default: return nil
            }
    ...
    static func parse(raw: [UInt8], isGS2Header: Bool = false) -> [SCRAMAttribute]? {
        ...
        let results = likelyAttributePairs.map {
            parseAttributePair(name: Array($0[0]), value: $0.dropFirst().first.map { Array($0) } ?? [], isGS2Header: isGS2Header)
        }
    ...
    static func serialize(_ attributes: [SCRAMAttribute], isInitialGS2Header: Bool = true) -> [UInt8]? {
        ...
        for attribute in attributes {
            ...
            case .r(let nonce):
                ...
                result.append(.equals)
                result.append(contentsOf: nonce.utf8.map { UInt8($0) })
            case .n(let name):
                result.append(UInt8(ascii: "n"))
                result.append(.equals)
                result.append(contentsOf: name.encodedAsSaslName.utf8.map { UInt8($0) })
            case .s(let salt):
                result.append(UInt8(ascii: "s"))
                result.append(.equals)
                result.append(contentsOf: salt.encodingBase64())
            case .i(let count):
                result.append(UInt8(ascii: "i"))
                result.append(.equals)
                result.append(contentsOf: "\(count)".utf8.map { UInt8($0) })
            case .p(let proof):
                result.append(UInt8(ascii: "p"))
                result.append(.equals)
                result.append(contentsOf: proof.encodingBase64())
            case .v(let signature):
                result.append(UInt8(ascii: "v"))
                result.append(.equals)
                result.append(contentsOf: signature.encodingBase64())
            case .e(let error):
                result.append(UInt8(ascii: "e"))
                result.append(.equals)
                result.append(contentsOf: error.rawValue.utf8.map { UInt8($0) })
            case .c(let binding, let identity):
                if isInitialGS2Header {
                    ...
                    case .bind(let name, _):
                        result.append(UInt8(ascii: "p"))
                        result.append(.equals)
                        result.append(contentsOf: name.utf8.map { UInt8($0) })
                    }
                    result.append(.comma)
                    if let identity = identity {
                        result.append(UInt8(ascii: "a"))
                        result.append(.equals)
                        result.append(contentsOf: identity.encodedAsSaslName.utf8.map { UInt8($0) })
                    }
                    result.append(.comma)
                } else {
                    guard var partial = serialize([attribute], isInitialGS2Header: true) else { return nil }
                    if case .bind(_, let data) = binding {
                        guard let data = data else { return nil }
                        partial.append(contentsOf: data)
                    }
                    result.append(UInt8(ascii: "c"))
                    result.append(.equals)
                    result.append(contentsOf: partial.encodingBase64())
                }
            default:
                return nil
            }
    ...
    internal enum SCRAM {
        ...
        internal struct SHA256: SASLAuthenticationMechanism {
            ...
            internal init(username: String, password: @escaping () throws -> String) {
                self._impl = .init(
                    username: username, passwordGrabber: { _ in try (Array(password().utf8), []) }, bindingInfo: .unsupported)
            }
        ...
        internal struct SHA256_PLUS: SASLAuthenticationMechanism {
            ...
            internal init(
                username: String, password: @escaping () throws -> String, channelBindingName: String, channelBindingData: [UInt8]
            ) {
                self._impl = .init(
                    username: username, passwordGrabber: { _ in try (Array(password().utf8), []) },
                    bindingInfo: .bind(channelBindingName, channelBindingData))
            }
...
private final class SASLMechanism_SCRAM_SHA256_Common {
    ...
    private enum State {
        ...
        case serverSentFirstMessage(
            clientBareFirstMessage: [UInt8], nonce: String, binding: SCRAMAttribute.GS2ChannelBinding, saltedPassword: [UInt8],
            serverFirstMessage: [UInt8])
        case serverDone
    }
    ...
    public func step(message: [UInt8]?) -> SASLAuthenticationStepResult {
        do {
            ...
            case .clientSentFirstMessage(let username, let nonce, let binding, let firstMessageBare):
                ...
                return try self.handleClientSentFirst(
                    message: serverFirstMessage, username: username, nonce: nonce, binding: binding, firstMessageBare: firstMessageBare)
            ...
            case .serverInitial(let extraNonce, let iterationCount, let bindingRequired):
                return try self.handleServerInitial(
                    message!, extraNonce: extraNonce, iterationCount: iterationCount, bindingRequired: bindingRequired)
            case .serverSentFirstMessage(
                let clientBareFirstMessage, let nonce, let previousBinding, let saltedPassword, let serverFirstMessage):
                return try self.handleServerSentFirst(
                    message!, clientBareFirstMessage: clientBareFirstMessage, nonce: nonce, previousBinding: previousBinding,
                    saltedPassword: saltedPassword, serverFirstMessage: serverFirstMessage)
            case .serverDone:
                throw SASLAuthenticationError.resultAlreadyDelivered
            }
        } catch {
    ...
    private func handleClientInitial(username: String, nonce: String, binding: SCRAMAttribute.GS2ChannelBinding) throws
    ...
    private func handleClientSentFirst(
        message: [UInt8], username: String, nonce: String,
        binding: SCRAMAttribute.GS2ChannelBinding, firstMessageBare: [UInt8]
        ...
        guard case .r(let serverNonce) = incomingAttributes.dropFirst(0).first else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }
        guard case .s(let serverSalt) = incomingAttributes.dropFirst(1).first else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }
        guard case .i(let serverIterations) = incomingAttributes.dropFirst(2).first else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }

        // Generate a `client-final-message-no-proof`
        guard let clientFinalNoProof = SCRAMMessageParser.serialize([.c(binding: binding), .r(serverNonce)], isInitialGS2Header: false)
        else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }
        ...
        var authMessage = firstMessageBare
        authMessage.append(.comma)
        authMessage.append(contentsOf: message)
        authMessage.append(.comma)
        authMessage.append(contentsOf: clientFinalNoProof)
        ...
        var clientFinalMessage = clientFinalNoProof
        clientFinalMessage.append(.comma)
        guard let proofPart = SCRAMMessageParser.serialize([.p(Array(clientProof))]) else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }
    ...
    private func handleClientSentFinal(message: [UInt8], saltedPassword: [UInt8], authMessage: [UInt8]) throws
        -> SASLAuthenticationStepResult
    {
        // Parse incoming
        guard let incomingAttributes = SCRAMMessageParser.parse(raw: message) else {
    ...
    private func handleServerInitial(
        _ message: [UInt8], extraNonce: String,
        iterationCount: UInt32, bindingRequired: Bool
    ) throws -> SASLAuthenticationStepResult {
        ...
        case .gp(.unused):
            if bindingRequired {
                throw SCRAMServerError.serverDoesSupportChannelBinding
            } else {
                throw SCRAMServerError.serverErrorValueExt("channel-bindings-expected-from-client")
            }
        ...
        guard case .gm(let clientFirstMessageBare) = channelAttributes.first, channelAttributes.count == 1,
            let incomingAttributes = SCRAMMessageParser.parse(raw: clientFirstMessageBare),
            case .n(let username) = incomingAttributes.dropFirst(0).first,
            case .r(let clientNonce) = incomingAttributes.dropFirst(1).first
        else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }
        ...
        // Save state and send
        self.state = .serverSentFirstMessage(
            clientBareFirstMessage: clientFirstMessageBare, nonce: clientNonce + extraNonce, binding: binding,
            saltedPassword: saltedPassword, serverFirstMessage: serverFirstMessage)
    ...
    private func handleServerSentFirst(
        ...
        guard case .c(let binding, _) = incomingAttributes.dropFirst(0).first,
            case .r(let repeatNonce) = incomingAttributes.dropFirst(1).first,
            case .p(let proof) = incomingAttributes.last
        else {
            throw SASLAuthenticationError.genericAuthenticationFailure
        }
        ...
        let storedKey = SHA256.hash(data: Data(clientKey))
        var authMessage = clientBareFirstMessage
        authMessage.append(.comma)
        authMessage.append(contentsOf: serverFirstMessage)
        authMessage.append(.comma)
        authMessage.append(contentsOf: message.dropLast(proof.count + 3))
        let clientSignature = HMAC<SHA256>.authenticationCode(for: authMessage, using: .init(data: storedKey))
