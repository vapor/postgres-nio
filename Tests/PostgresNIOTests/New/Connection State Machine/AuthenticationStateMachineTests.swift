...
@Suite struct AuthenticationStateMachineTests {
    ...
    @Test func testAuthenticateMD5WithoutPassword() {
        ...
        #expect(state.provideAuthenticationContext(authContext) == .sendStartupMessage(authContext))
        #expect(
            state.authenticationMessageReceived(.md5(salt: salt))
    ...
    @Test func testAuthenticateSCRAMSHA256WithAtypicalEncoding() {
        ...
        let saslContinueResponse = state.authenticationMessageReceived(
            .saslContinue(
                data: .init(
                    bytes:
    ...
    @Test func testUnsupportedAuthMechanism() {
        ...
        for (message, mechanism) in unsupported {
            ...
            #expect(state.provideAuthenticationContext(authContext) == .sendStartupMessage(authContext))
            #expect(
                state.authenticationMessageReceived(message)
                    == .closeConnectionAndCleanup(
                        .init(action: .close, tasks: [], error: .unsupportedAuthMechanism(mechanism), closePromise: nil)))
        }
    }
    ...
    @Test func testUnexpectedMessagesAfterStartUp() {
        ...
        for message in unexpected {
            ...
            #expect(
                state.authenticationMessageReceived(message)
                    == .closeConnectionAndCleanup(
                        .init(action: .close, tasks: [], error: .unexpectedBackendMessage(.authentication(message)), closePromise: nil)))
        }
    }
