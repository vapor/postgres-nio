...
@Suite struct ExtendedQueryStateMachineTests {
    @Test func testExtendedQueryWithoutDataRowsHappyPath() throws {
        ...
        #expect(
            state.commandCompletedReceived("DELETE 1")
                == .succeedQuery(promise, with: .init(value: .noRows(.tag("DELETE 1")), logger: logger)))
    ...
    @Test func testExtendedQueryWithDataRowsHappyPath() throws {
        ...
        let input: [RowDescription.Column] = [
            .init(
                name: "version", tableOID: 0, columnAttributeNumber: 0, dataType: .text, dataTypeSize: -1, dataTypeModifier: -1,
                format: .text)
        ]
        let expected: [RowDescription.Column] = input.map {
            .init(
                name: $0.name, tableOID: $0.tableOID, columnAttributeNumber: $0.columnAttributeNumber, dataType: $0.dataType,
    ...
    @Test func testReceiveTotallyUnexpectedMessageInQuery() throws {
        ...
        #expect(
            state.authenticationMessageReceived(.ok)
                == .failQuery(
                    promise, with: psqlError, cleanupContext: .init(action: .close, tasks: [], error: psqlError, closePromise: nil)))
    }
