...
@Suite struct PrepareStatementStateMachineTests {
    @Test func testCreatePreparedStatementReturningRowDescription() throws {
        ...
        #expect(
            state.enqueue(task: .extendedQuery(prepareStatementContext))
        ...
        let columns: [RowDescription.Column] = [
            .init(
                name: "id", tableOID: 0, columnAttributeNumber: 0, dataType: .int8, dataTypeSize: 8, dataTypeModifier: -1, format: .binary)
        ]

        #expect(
            state.rowDescriptionReceived(.init(columns: columns))
    ...
    func testCreatePreparedStatementReturnsBinaryFormattedRowDescription() throws {
        ...
        let textColumns: [RowDescription.Column] = [
            .init(
                name: "id", tableOID: 0, columnAttributeNumber: 0, dataType: .int8, dataTypeSize: 8, dataTypeModifier: -1, format: .text
            )
        ]
        ...
        #expect(
            state.rowDescriptionReceived(.init(columns: textColumns))
    ...
    @Test func testCreatePreparedStatementReturningNoData() throws {
        ...
        #expect(state.noDataReceived() == .succeedPreparedStatementCreation(promise, with: nil))
        #expect(state.readyForQueryReceived(.idle) == .fireEventReadyForQuery)
    }
    ...
    @Test func testErrorReceivedAfter() throws {
        ...
        #expect(
            state.authenticationMessageReceived(.ok)
                == .closeConnectionAndCleanup(
                    .init(action: .close, tasks: [], error: .unexpectedBackendMessage(.authentication(.ok)), closePromise: nil)))
    }
}
