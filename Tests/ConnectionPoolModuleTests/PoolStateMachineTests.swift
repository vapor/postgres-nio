...
@Suite struct PoolStateMachineTests {
    ...
    @available(macOS 13.0, iOS 16.0, tvOS 16.0, watchOS 9.0, *)
    @Test func testGracefulShutdownCompletesWhenConnectionFailsAfterLastRequestIsCancelled() {
        struct ConnectionFailed: Error {}
        var configuration = PoolConfiguration()
        configuration.minimumConnectionCount = 0
        configuration.maximumConnectionSoftLimit = 1
        ...
        // enqueue request for connection creation
        let request1 = MockRequest(connectionType: MockConnection.self)
        ...
        // cancel request while connection is starting
        let cancelAction = stateMachine.cancelRequest(id: request1.id)
        #expect(cancelAction.request == .failRequest(request1, .requestCancelled))
        #expect(cancelAction.connection == .none)

        // when starting connection fails there is nothing to retry for, shutdown
        let failedAction = stateMachine.connectionEstablishFailed(ConnectionFailed(), for: connectionRequest)
        #expect(failedAction.request == .none)
    ...
    /// Graceful shutdown never force-cleans connections: every connection is disposed of
    /// lazily, by its own next event. This test assembles every connection state that can
    /// coexist with a draining request queue - keep-alive-running, leased, starting, and
    /// backing-off - triggers a graceful shutdown, and walks each connection's disposal
    /// event, verifying the pool completes without any sweep.
    /// (Parked idle connections cannot coexist with a non-empty queue; they are reaped
    /// eagerly by the trigger itself, see `testTriggerGracefulShutdownWithIdleConnections`.)
    @available(macOS 13.0, iOS 16.0, tvOS 16.0, watchOS 9.0, *)
    @Test func testGracefulShutdownDisposesAllConnectionStates() {
        struct ConnectionFailed: Error {}
        var configuration = PoolConfiguration()
        configuration.minimumConnectionCount = 0
        configuration.maximumConnectionSoftLimit = 4
        ...
        // Connection A: established and leased, serving request1.
        let request1 = MockRequest(connectionType: MockConnection.self)
        guard case .makeConnection(let connectionRequestA, _) = stateMachine.leaseConnection(request1).connection else { Issue.record(); return }
        let connectionA = MockConnection(id: connectionRequestA.connectionID)
        #expect(stateMachine.connectionEstablished(connectionA, maxStreams: 1).request == .leaseConnection(.init(element: request1), connectionA))

        // Connection K: established, parked, then its keep-alive starts running. With
        // `keepAliveReducesAvailableStreams` the keep-alive consumes K's only stream, so K
        // is idle-shaped but not leasable - the one idle variant that survives a
        // non-empty-queue trigger.
        let requestK = MockRequest(connectionType: MockConnection.self)
        guard case .makeConnection(let connectionRequestK, _) = stateMachine.leaseConnection(requestK).connection else { Issue.record(); return }
        let connectionK = MockConnection(id: connectionRequestK.connectionID)
        #expect(stateMachine.connectionEstablished(connectionK, maxStreams: 1).request == .leaseConnection(.init(element: requestK), connectionK))
        let parkAction = stateMachine.releaseConnection(connectionK, streams: 1)
        guard case .scheduleTimers(let parkTimers) = parkAction.connection,
            let keepAliveTimer = Array(parkTimers).first(where: { $0.underlying.usecase == .keepAlive }),
            let idleTimer = Array(parkTimers).first(where: { $0.underlying.usecase == .idleTimeout })
        else {
            Issue.record("Expected keep-alive and idle timers to be scheduled, got \(parkAction.connection)")
            return
        }
        #expect(stateMachine.timerScheduled(keepAliveTimer, cancelContinuation: MockTimerCancellationToken(keepAliveTimer)) == .none)
        let idleTimerToken = MockTimerCancellationToken(idleTimer)
        #expect(stateMachine.timerScheduled(idleTimer, cancelContinuation: idleTimerToken) == .none)
        let keepAliveTriggerAction = stateMachine.timerTriggered(keepAliveTimer)
        guard case .runKeepAlive(let keepAliveConnection, _) = keepAliveTriggerAction.connection, keepAliveConnection === connectionK else {
            Issue.record("Expected keep-alive to run on connection K, got \(keepAliveTriggerAction.connection)")
            return
        }

        // Requests 2-4 queue up. Request 2 is absorbed by K counting as soon-available
        // (its keep-alive will finish); requests 3 and 4 spawn connections B and S.
        let request2 = MockRequest(connectionType: MockConnection.self)
        #expect(stateMachine.leaseConnection(request2) == .none())
        let request3 = MockRequest(connectionType: MockConnection.self)
        guard case .makeConnection(let connectionRequestB, _) = stateMachine.leaseConnection(request3).connection else { Issue.record(); return }
        let request4 = MockRequest(connectionType: MockConnection.self)
        guard case .makeConnection(let connectionRequestS, _) = stateMachine.leaseConnection(request4).connection else { Issue.record(); return }

        // Connection B fails to establish and backs off; connection S is still starting.
        let failedAction = stateMachine.connectionEstablishFailed(ConnectionFailed(), for: connectionRequestB)
        guard case .scheduleTimers(let backoffTimers) = failedAction.connection, let backoffTimer = Array(backoffTimers).first else { Issue.record(); return }
        let backoffToken = MockTimerCancellationToken(backoffTimer)
        #expect(stateMachine.timerScheduled(backoffTimer, cancelContinuation: backoffToken) == .none)

        // The full menagerie: A leased, K running keep-alive, S starting, B backing off,
        // three requests queued. The trigger only latches - nothing is reaped.
        #expect(stateMachine.triggerGracefulShutdown() == .none())
        #expect(!stateMachine.isShutdown)

        // Disposal 1 - starting: S establishes and is leased straight to a queued request.
        let connectionS = MockConnection(id: connectionRequestS.connectionID)
        #expect(stateMachine.connectionEstablished(connectionS, maxStreams: 1).request == .leaseConnection(.init(element: request2), connectionS))

        // Disposal 2 - keep-alive-running: K's keep-alive completes and K rejoins service,
        // taking the next queued request; its idle timer is cancelled by the lease.
        let keepAliveDoneAction = stateMachine.connectionKeepAliveDone(connectionK)
        #expect(keepAliveDoneAction.request == .leaseConnection(.init(element: request3), connectionK))
        #expect(keepAliveDoneAction.connection == .cancelTimers([idleTimerToken]))
        #expect(!stateMachine.isShutdown)

        // Disposal 3 - leased: connection A's release drains the last queued request; the
        // queue empties inside this event, so the pool moves to `shuttingDown`.
        #expect(stateMachine.releaseConnection(connectionA, streams: 1).request == .leaseConnection(.init(element: request4), connectionA))

        // Every subsequent release closes its connection; the pool is not done while
        // others remain.
        #expect(stateMachine.releaseConnection(connectionA, streams: 1).connection == .closeConnection(connectionA, []))
        #expect(stateMachine.connectionClosed(connectionA).connection == .cancelTimers([]))
        #expect(stateMachine.releaseConnection(connectionS, streams: 1).connection == .closeConnection(connectionS, []))
        #expect(stateMachine.connectionClosed(connectionS).connection == .cancelTimers([]))
        #expect(stateMachine.releaseConnection(connectionK, streams: 1).connection == .closeConnection(connectionK, []))
        #expect(stateMachine.connectionClosed(connectionK).connection == .cancelTimers([]))
        #expect(!stateMachine.isShutdown)

        // Disposal 4 - backing-off: B is the last occupant. Its backoff timer firing is its
        // disposal event, and completes the shutdown.
        let backoffDoneAction = stateMachine.timerTriggered(backoffTimer)
        #expect(backoffDoneAction.request == .none)
        #expect(backoffDoneAction.connection == .cancelEventStreamAndFinalCleanup([backoffToken]))
        #expect(stateMachine.isShutdown)
